...
def get_account_from_eat(eat_token):
    try:
        ...
        match = re.search(r'([A-Za-z0-9\-_]+\.[A-Za-z0-9\-_]+\.[A-Za-z0-9\-_]+)', eat_token)
        if match:
            eat_token = match.group(1)

        url = "https://eat-api.thory.buzz/api"
        response = requests.get(f"{url}?eatjwt={eat_token}", timeout=15)

        if response.status_code != 200:
            return None, None, "Invalid token"

        data = response.json()
        return data.get("jwt"), data.get("account"), None
